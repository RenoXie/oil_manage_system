...
router.post('/', requirePermission('categories'), async (req, res) => {
  try {
    const { name } = req.body;
    if (!name) return res.status(400).json({ code: 400, msg: '类别名称不能为空' });
    if (typeof name !== 'string' || name.length > 50) return res.status(400).json({ code: 400, msg: '类别名称最长50字符' });

    const exists = await db('oil_categories').where({ name, deletestatus: 0 }).first();
    if (exists) return res.status(400).json({ code: 400, msg: '类别已存在' });
...
router.put('/:id', requirePermission('categories'), async (req, res) => {
  try {
    const { name } = req.body;
    if (name === undefined) return res.status(400).json({ code: 400, msg: '类别名称不能为空' });
    if (typeof name !== 'string' || name.length > 50) return res.status(400).json({ code: 400, msg: '类别名称最长50字符' });

    const record = await db('oil_categories').where({ id: req.params.id, deletestatus: 0 }).first();
    if (!record) return res.status(404).json({ code: 404, msg: '类别不存在' });

    const exists = await db('oil_categories').where({ name, deletestatus: 0 }).whereNot({ id: req.params.id }).first();
    if (exists) return res.status(400).json({ code: 400, msg: '类别名已存在' });
...
router.delete('/:id', requirePermission('categories'), async (req, res) => {
  try {
    ...
    const hasStock = await db('stock_in').where({ oil_category_id: req.params.id, deletestatus: 0 }).first()
      || await db('stock_out').where({ oil_category_id: req.params.id, deletestatus: 0 }).first();
    if (hasStock) return res.status(400).json({ code: 400, msg: '该类别有关联的出入库记录，无法删除' });
