...
router.get('/overview', async (req, res) => {
  try {
    ...
    if (isCustomer) outQuery.where('customer_id', custId || 0);
    ...
    console.error(`[${new Date().toISOString()}] GET /api/statistics/overview |`, err.message);
...
router.get('/buyer', async (req, res) => {
  try {
    ...
    const records = await query
      .select('stock_out.*', 'oil_categories.name as category_name', 'vehicles.plate_number')
    ...
    console.error(`[${new Date().toISOString()}] GET /api/statistics/buyer |`, err.message);
    res.status(500).json({ code: 500, msg: '服务器内部错误' });
  }
});
...
router.get('/buyers', async (req, res) => {
  try {
    ...
    let total = { liters: 0, amount: 0, times: 0 };
    for (const item of list) {
      total.liters = toMoney(total.liters + +item.total_liters);
      total.amount = toMoney(total.amount + +item.total_amount);
      total.times += item.times;
    }

    res.json({ code: 0, data: { list, total } });
  } catch (err) {
    console.error(`[${new Date().toISOString()}] GET /api/statistics/buyers |`, err.message);
    res.status(500).json({ code: 500, msg: '服务器内部错误' });
  }
});
